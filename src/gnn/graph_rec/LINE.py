...
def create_model(numNodes, embedding_dim, order='all'):
    ...
    # 两个顶点同时出现的联系概率
    join_prob = tf.sigmoid(tf.reduce_sum(v_i_embed * v_j_embed, axis=-1, keepdims=False))
    # 由中心节点得到背景节点的条件概率
    cond_prob = tf.sigmoid(tf.reduce_sum(v_i_embed_second * v_j_embed_context, axis=-1, keepdims=False))
    if order == 'first':
        outputs = [join_prob]
    elif order == 'second':
        outputs = [cond_prob]
...
def kl_dist(y_true, y_pred):
    '''
    在一阶相似度中，y_true为1；在二阶相似度中，正样本y_true=1，负样本y_true=-1
    '''
...
class LINE(object):
    def __init__(self, graph, nodesNum, batch_size, embedding_dim=100, negative_k=5, order='all'):
        ...
        self.model.compile(optimizer, loss=kl_dist)  # 自定义损失函数
    ...
    def _batch_generator(self):
        ...
        while True:  # 这是个无限循环，可以一直生成样本
            ...
            for i in range(self.negative_k):
                ...
                if self.order == 'all':
                    yield ([np.array(source), np.array(sink)], [y, y])
                else:
                    yield ([np.array(source), np.array(sink)], [y])
    ...
    def train(self, epochs):
        ...
        self.model.fit(self._batch_generator(), steps_per_epoch=steps_per_epoch, epochs=epochs)  # 可以把样本做成迭代器传给fit函数
...
if __name__ == "__main__":
    line, node2index = train_epinions("data/ratings_data.txt", 20, 'second')
    test(line, node2index, 1, 10)
    test(line, node2index, 1, 11)
    test(line, node2index, 1, 12)
    test(line, node2index, 1, 13)
