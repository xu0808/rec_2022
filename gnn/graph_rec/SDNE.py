...
from tensorflow.keras.layers import Dense, Embedding, Input, Reshape
...
def sample_generator(edges, adjacency_matrix, embedding_dim, batch_size=32):
    ...
    for step in range(math.ceil(1.0 * N / batch_size)):
        ...
        yield ([v_i, v_j], [x_i, x_j, dummy_output])  # 输入：相邻的2个顶点的编号。输出：这2个顶点的重构向量
...
def create_model(alpha, beta, node_num, hidden_dims, v, adjacency_matrix):
    ...
    fcs = [embedding_layer, Reshape(
        target_shape=(node_num,))]  # 在定义Layer的时候是不考虑batch_size的。input经过embedding_layer会多出一维，通过Reshape把多出的这一维squeeze掉
    ...
    y_i = encoder(input_i)  # encode结果就是结点的向量表示
    x_i_hat = decoder(y_i)
    y_j = encoder(input_j)
    x_j_hat = decoder(y_j)
    ...
    model = Model(inputs=[input_i, input_j],
                  outputs=[x_i_hat, x_j_hat, y_diff])
    model.compile(loss=[reconstruction_loss(beta), reconstruction_loss(
        beta), embedding_loss()], loss_weights=[1, 1, alpha])  # 二阶相似度权重为1，一阶相似度权重为alpha
